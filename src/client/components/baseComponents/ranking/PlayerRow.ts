...
export class PlayerRow extends LitElement {
    ...
    return html`
      <li
        class="bg-gradient-to-r ${player.winner
          ...
          relative pt-1 pb-1 pr-5 pl-5 mb-[5px] rounded-lg flex justify-between items-center hover:bg-slate-500 transition duration-150 ease-in-out"
      >
        <div
  ...
  private renderCrownIcon() {
    return html`<img
      ...
      class="absolute top-[-1px] left-[28px] w-[15px] h-[15px] sm:top-[-7px] sm:left-[30px] sm:w-[20px] sm:h-[20px]"
    />`;
  }
  ...
  private renderScoreBar() {
    const width = (this.score / this.bestScore) * 100;
    return html`
      <div class="w-full pr-[10px] m-auto">
        <div class="h-[9px] bg-neutral-800" style="width: 100%;">
          <!-- bar background -->
          <div class="h-[9px] bg-white" style="width: ${width}%;"></div>
        </div>
      </div>
    `;
  }
  ...
  private renderBombs() {
    return html`
      <div class="flex justify-between text-sm sm:pr-20">
        <div
          class="${this.rankType === RankType.Atoms
            ? "font-bold text-[18px]"
        ...
        /
        <div
          class="${this.rankType === RankType.Hydros
            ? "font-bold text-[18px]"
            : ""} min-w-[30px] sm:min-w-[60px] inline-block text-center"
        >
          ${this.player.hydros}
        ...
        /
        <div
          class="${this.rankType === RankType.MIRV
            ? "font-bold text-[18px]"
            : ""} min-w-[30px] sm:min-w-[60px] inline-block text-center"
        >
          ${this.player.mirv}
  ...
  private renderBombScore() {
    return html`
      <div class="flex gap-3 items-center w-full">
        ${this.renderPlayerIcon()}
        <div class="flex flex-col sm:flex-row gap-1 text-left w-full">
          ${this.renderPlayerName()} ${this.renderBombs()}
        </div>
      </div>
    `;
  }
  ...
  private renderIcon() {
    if (this.player.killedAt) {
      ...
      return html`<img src="${"/flags/" + this.player.flag + ".svg"}" class="w-[30px] h-[30px] sm:w-[40px] sm:h-[40px]"></img>`;
    }
