...
export class Ranking {
  ...
  private summarizePlayers(session: AnalyticsRecord): PlayerInfo[] {
    ...
    for (const player of session.info.players) {
      ...
      const username = match ? match[2] : player.username;
      const gold = (player.stats!.gold ?? []).map((v) => BigInt(v ?? 0));
      players[player.clientID] = {
        id: player.clientID,
        rawUsername: player.username,
        username,
        tag: player.clanTag,
        conquests: Number(player.stats!.conquests) || 0,
        flag: player.cosmetics?.flag ?? undefined,
        killedAt: player.stats!.killedAt ?? undefined,
        gold,
        atoms: Number(player.stats!.bombs?.abomb?.[0]) || 0,
        hydros: Number(player.stats!.bombs?.hbomb?.[0]) || 0,
        mirv: Number(player.stats!.bombs?.mirv?.[0]) || 0,
        winner: false,
      };
    }
